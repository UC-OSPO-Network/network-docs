...
The $8.8 trillion figure is from Hoffmann, Nagle & Zhou (2024), "The Value of Open Source Software," Harvard Business School Working Paper 24-038. Supply-side cost to recreate the top 5,000 most-used packages: $4.15 billion. Demand-side replacement value to global enterprises if OSS disappeared: $8.8 trillion. That ratio - roughly 2,000x - is the clearest way to explain why open source governance matters to an administrator or funder who thinks this is a niche concern.
:::
...
<div style="font-size:0.6em; margin-top:0.3em; line-height:1.4;">of UC OSS contributors<br>are also <strong>maintainers</strong><br><span style="opacity:0.7;">not just users, stewards</span></div>
</div>
...
::: {.notes}
The old framing was "intra vs. inter campus" as if these were competing priorities. The better frame: each campus is a node that both contributes and draws from the network. UC Davis specializes in software sustainability research. UCLA brings library integration and data services expertise. UCSD leads on educational outreach. UCSC has the deepest OSS governance history through CROSS. That distributed expertise is the asset - the network is how you move it around.

The practical governance: OSPO Leadership Group (one rep per campus) sets direction; three thematic working groups (Education, Sustainability, Compliance) do the work; GitHub-based project management keeps it open and legible.

Key point for this audience: a librarian or data professional at a smaller campus now has access to the expertise of six campuses. That is what the network model actually delivers.
:::
...
::: {style="background:#1f335e; border-radius:8px; padding:1.2em 1em; color:white; margin-bottom:0.7em;"}
...
::: {style="background:#f0f4fa; border-radius:8px; padding:0.9em 1em; font-size:0.78em;"}
**OSPO heuristics already in your toolkit:**

health signals · bus factor · governance files · FAIR4RS · software DMPs · contributor conventions · community governance
:::
...
::: {.notes}
The core of this slide is the distinction between reactive and capacity-building service. The goal is not to turn every data librarian into an OSPO director. The goal is to shift what a researcher walks away from a consultation knowing. If they leave with a license file and a CITATION.cff, that is a one-time fix. If they leave understanding *why* those things matter - for reproducibility, for credit, for compliance - that is a shift in practice that compounds across their lab, their students, their next grant.

The OSPO heuristics are not exotic. Health signals (bus factor, issue velocity, contributor diversity) tell you whether a project will be there next year. Governance files (README, CONTRIBUTING, CODE_OF_CONDUCT) are the equivalent of a data README - they make the project legible to the next person. FAIR4RS and software DMPs are the policy anchors that give researchers a reason to care now.

"Put up your shingle" - you don't need institutional approval to start doing this work in your consultations. Use the vocabulary, the frameworks, the resources from ucospo.net. You are already in the room where these questions get raised. Be the person who has the answer and the frame.

The student angle: if you run Carpentries workshops or data services instruction, adding a session on software health, licensing, and CITATION.cff is a 1σ move for every researcher in that room.
:::
...
📄 One-page reference handout (project origins, network stats, resources): [iassist-2026-handout.html](iassist-2026-handout.html)
:::
